...
def _fact(fact_id: str, value: str, as_of: str, source: SourceType) -> Fact:
    return Fact(
        id=fact_id, tenant_id="t", entity="ATLS", metric="total_revenue", period="FY2026-Q1",
...
def test_versions_sorted_and_latest_resolves_restatement():
    ...
    versions = store.versions("t", "ATLS", "total_revenue", "FY2026-Q1")
    assert [v.as_of for v in versions] == ["2025-04-28", "2025-09-15"]
    assert store.latest("t", "ATLS", "total_revenue", "FY2026-Q1").value == Decimal("474")
...
def test_same_fact_id_isolated_across_tenants():
    ...
    store = InMemoryFactStore()
    a = Fact(
        id="acc:total_revenue:FY2026-Q1:2026-04-28", tenant_id="alpha", entity="ATLS",
...
def test_demo_ingests_into_multiple_tenants():
    ...
    store = InMemoryFactStore()
    for tenant in ("atlas", "acme", "brandnew"):
        facts, _ = XBRLConnector().fetch(load_fixture("atlas_q1_fy2026"), tenant_id=tenant)
        store.add_many(facts)
    assert len(store.all("atlas")) == len(store.all("acme")) > 0


def test_xbrl_ingestion_maps_tags_and_precision():
    store = InMemoryFactStore()
    facts, report = XBRLConnector().fetch(load_fixture("atlas_q1_fy2026"), tenant_id="atlas")
    store.add_many(facts)

    rev = store.latest("atlas", "ATLS", "total_revenue", "FY2026-Q1")
    ...
    versions = store.versions("atlas", "ATLS:Cloud", "cloud_growth_yoy", "FY2026-Q1")
...
def test_unmapped_tags_are_skipped_not_guessed():
    instance = {"accession": "x", "entity": "ATLS", "facts": [
...
def test_guidance_is_not_filed():
    facts, _ = XBRLConnector().fetch(load_fixture("atlas_q1_fy2026"), tenant_id="atlas")
